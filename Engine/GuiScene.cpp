...
void GuiScene::Draw()
{
	...
	if (ImGui::Begin("Scene", &visible, ImGuiWindowFlags_MenuBar))
	{
		...
		if (App->gui->sceneRenderSize.x != window_size.x || App->gui->sceneRenderSize.y != window_size.y)
			App->gui->ScreenResized(window_size);
		...
		//ImGui::Image((ImTextureID)App->renderer3D->depthTexture, sceneRenderSize, ImVec2(0.0f, 1.0f), ImVec2(1.0f, 0.0f));
	}
	ImGui::End();
	ImGui::PopStyleVar();
}
